...
import { appsPageContent, type AppsPageContent } from "@/content/pages.content";
import ImmersiveStage from "./ImmersiveStage";
import { immersiveAppsConfig } from "./immersive.config";
import styles from "./ImmersiveHero.module.css";
import appsStyles from "./ImmersiveAppsHero.module.css";
...
/**
 * Apps listing hero with the engine's "App Nexus" preset (`immersiveExperience`).
 *
 * Same structure as the other immersive heroes: the badge, page title (the
 * only h1 on /apps), description and CTA are server-rendered HTML; the engine
 * mounts into the decorative stage on the client. Static states (reduced
 * motion, no WebGL, engine fallback) show the engine's app-nexus illustration
 * instead of the particles.
 */
export default function ImmersiveAppsHero({ content = appsPageContent }: Props) {
  const heroStyle = { "--immersive-scroll-length": immersiveAppsConfig.scrollLength } as CSSProperties;

  return (
    <section
      data-immersive-hero=""
      aria-labelledby="immersive-apps-title"
      className={`${styles.hero} bg-surface-0 text-ink`}
      style={heroStyle}
    >
      <div className={styles.viewport}>

        {/* Static aurora glow: first paint and background behind the fallback. */}
        <div aria-hidden="true" className={appsStyles.glow}></div>

        <ImmersiveStage className={styles.stage} settings={immersiveAppsConfig.engine} />
